...
export class MinioService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.s3Client = new S3Client({
      region: "us-east-1",
      endpoint: this.configService.get<string>("AWS_MINIO_ENDPOINT") || "http://minio:9000",
      credentials: {
    ...
    this.ensureBucketExists()
      .then(() => this.logger.log(`Bucket '${this.bucketName}' is ready.`))
      .catch(err => this.logger.error(`Failed to ensure bucket exists: ${err.message}`));
  }
  private async ensureBucketExists(): Promise<void> {
    try {
      await this.s3Client.send(new HeadBucketCommand({ Bucket: this.bucketName }));
      this.logger.log(`Bucket '${this.bucketName}' already exists. ${new Date()}`);
    } catch (err) {
